...
def moving_average(df):
    df["Open Rolling Mean 50 days"] = (
        df.groupby("Stock")["Open"].rolling(50).mean().reset_index(level=0, drop=True)
    )
    df["Open Rolling Mean 200 days"] = (
        df.groupby("Stock")["Open"].rolling(200).mean().reset_index(level=0, drop=True)
    )

    df["Open Rolling Mean 10 days"] = (
        df.groupby("Stock")["Open"].rolling(10).mean().reset_index(level=0, drop=True)
    )
    df["Open Rolling Mean 40 days"] = (
        df.groupby("Stock")["Open"].rolling(40).mean().reset_index(level=0, drop=True)
    )

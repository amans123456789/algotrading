...
def read_csvs(file_paths):
    dfs = {}
    if check_all_conditions(file_paths):
        for key, path in file_paths.items():
            if os.path.exists(path):
                dfs[key] = pd.read_csv(path)
                logging.info(f"Reading CSV at path: {path}")
            else:
                logging.error(f"CSV not found at path: {path}")

        return dfs
    else:
        logging.info(f"All csv files not found")



def create_file_paths(date, fund_analysis_year, ratio_analysis_date, news_date):
    file_paths = {
        "mark_cond_daily": f"Source_Data/Market_Condition/daily_market_condition_{date}.csv",
        "mark_cond_minute": f"Source_Data/Market_Condition/hourly_market_condition_{date}.csv",
        "last_mark_cond_daily": f"Source_Data/Market_Condition/last_day_fibonacci_market_indicator_{date}.csv",
        "last_mark_cond_minute": f"Source_Data/Market_Condition/last_hour_fibonacci_market_indicator_{date}.csv",
        "lagging_ind_cond_daily": f"Source_Data/Lagging_Indicators/daily_lag_indicator_{date}.csv",
        "lagging_ind_cond_minute": f"Source_Data/Lagging_Indicators/hourly_lag_indicator_{date}.csv",
        "last_lagging_ind_cond_daily": f"Source_Data/Lagging_Indicators/last_day_fibonacci_lag_indicator_{date}.csv",
        "last_lagging_ind_cond_minute": f"Source_Data/Lagging_Indicators/last_hour_fibonacci_lag_indicator_{date}.csv",
        "fund_analysis": f"Source_Data/Fundamental_Analysis/fundamental_analysis_{fund_analysis_year}.csv",
        "ratio_analysis": f"Source_Data/Call_Put/call_put_ratio_{ratio_analysis_date}.csv",
        "news": f"Source_Data/News/news-sentiment-data_{news_date}.csv",
    }
    return file_paths
...
def check_all_conditions(file_paths):
    try:
        ...
        # Logging the check for Condition 3 (continued)
        logging.info(
            "Checking Condition 3 (continued): Dates of daily market condition and daily lagging indicators should match."
        )
        if lagging_ind_cond_daily_date != mark_cond_daily_date:
            logging.error("Dates of daily market condition and daily lagging indicators do not match.")
            raise ValueError("Dates of daily market condition and daily lagging indicators do not match.")
...
file_paths = {
    ...
    "lagging_ind_cond_daily": "Source_Data/Lagging_Indicators/daily_lag_indicator_2024-01-14.csv",
    "lagging_ind_cond_minute": "Source_Data/Lagging_Indicators/hourly_lag_indicator_2024-01-14.csv",
    "last_lagging_ind_cond_daily": "Source_Data/Lagging_Indicators/last_day_fibonacci_lag_indicator_2024-01-14.csv",
    "last_lagging_ind_cond_minute": "Source_Data/Lagging_Indicators/last_hour_fibonacci_lag_indicator_2024-01-14.csv",
